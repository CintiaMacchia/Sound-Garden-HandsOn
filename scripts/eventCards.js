...
const listEvents = async () => {
    ...
    for (let index = 0; index < 3; index++) {
        ...
        <h2>${responseContent[index].name} - ${datasFormatadas.toLocaleString('en-GB')}</h2>
        <h4>${responseContent[index].attractions}</h4>
        <p>${responseContent[index].description}</p>
        <a href="#" event-id="${responseContent[index]._id}" class="btn btn-primary">reservar ingresso</a>
    </article>`
    }
